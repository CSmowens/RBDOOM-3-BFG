...
void idMegaTexture::BindForViewOrigin( const idVec3 viewOrigin ) {
	...
	for ( int i = 0 ; i < 7 ; i++ ) {
		...
		if ( i >= numLevels ) {
			...
			SetVertexParm((renderParm_t)(RENDERPARM_USER + i), parms);
		} else {
			...
			SetVertexParm((renderParm_t)(RENDERPARM_USER + i), level->parms);
	...
	SetVertexParm((renderParm_t)(RENDERPARM_USER + 7), parms);
	...
	SetVertexParm((renderParm_t)(RENDERPARM_USER + 8), parms);
}
